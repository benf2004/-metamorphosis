local level = {
	title = "Fire in the Night",
	worm = {
		x = 100,
		y = 374,

	},
	foodTruck = {
		standardWorm = 96,
		shieldWorm = 2,
		clockWorm = 2
	},
	miniFireSpouts = {
	 {x = 512, y = 384, rotation = 0, rotate = true},
	 {x = 156, y = 384, rotation = 45, rotate = true},
	 {x = 889, y = 384, rotation = 90, rotate = true},
	 {x = 512, y = 384, rotation = 135, rotate = true},
	 {x = 512, y = 128, rotation = 190, rotate = true},
	 {x = 512, y = 612, rotation = 235, rotate = true},
	 {x = 889, y = 612, rotation = 280, rotate = true},
	 {x = 889, y = 128, rotation = 315, rotate = true},
	 {x = 156, y = 128, rotation = 360, rotate = true},
	 {x = 156, y = 612, rotation = 45, rotate = true} 
	},
	secondsAllowed = 60,
	lengthObjective = 25,
	sceneEffect = "midnight",
	instructions = "Yum!"
}

return level